...
export const getListMultiplicador = async (req: Request, res: Response) => {
  let data = req.query;
  await pool.query(
    "SELECT *from function_listar_multiplicador($1,$2, $3, $4, $5, $6);",
    [
      data.id_branch,
...
export const readMultiplicador = async (req: Request, res: Response) => {
  let data = req.query;
  await pool.query(
    "SELECT *from function_ver_multiplicador($1);",
    [data.id],
    (err, response, fields) => {
      if (!err) {
        let rows = response.rows;
...
export const updateMultiplicador = async (req: Request, res: Response) => {
  ...
  await pool.query(
    "SELECT *from function_actualizar_multiplicador($1,$2,$3,$4,$5, $6);",
    [
      data.id,
      data.name,
      data.description,
...
export const getCargarShipment = async (req: Request, res: Response) => {
  let data = req.query;
  await pool.query(
    "SELECT *from function_cargar_shipment($1);",
    [data.id_branch],
    (err, response, fields) => {
      if (!err) {
        let rows = response.rows;
