...
export const getListYear = async (req: Request, res: Response) => {
  ...
  await pool.query(
    "SELECT *from function_year_listar($1,$2, $3);",
    [
      data.id_branch,
...
export const insertYear = async (req: Request, res: Response) => {
  ...
  await pool.query(
    "SELECT *from function_year_insertar($1,$2, $3);",
...
export const readYear = async (req: Request, res: Response) => {
  ...
  await pool.query(
    "SELECT *from function_year_ver($1);",
    [data.id],
    (err, response, fields) => {
      if (!err) {
        let rows = response.rows;
...
export const updateYear = async (req: Request, res: Response) => {
  ...
  await pool.query(
    "SELECT *from function_year_actualizar($1,$2, $3);",
...
export const validateCodeInTableYear = async (req: Request, res: Response) => {
  ...
  await pool.query(
    "SELECT *from function_validar_codigo_year($1,$2);",
    [data.description, data.id_branch],
    (err, response, fields) => {
      if (!err) {
        let rows = response.rows;
        res.json({
