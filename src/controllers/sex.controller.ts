...
export const validateAcronymInTableSexNuevo = async (req: Request, res: Response) => {
  ...
  await pool.query("SELECT *from function_validar_acronimo_nuevo_sexo($1,$2);", [
    data.acronym,
    data.id_branch,
  ],
  (err, response, fields) => {
...
export const validateAcronymInTableSexEditar = async (req: Request, res: Response) => {
  const data = req.query;

  await pool.query("SELECT *from function_validar_acronimo_editar_sexo($1,$2, $3);", [
    data.id,
    data.acronym,
    data.id_branch,
  ],
